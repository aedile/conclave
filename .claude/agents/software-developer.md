...
Before writing any test, read `.claude/standards/test-quality.md` in full. It contains
Rules A-F (parametrize, no tautological assertions, no coverage gaming, fixture reuse,
file size limit, helpers over duplication). These are enforceable rules with programmatic
gates — not suggestions. Violations are FINDING-level in QA review.
