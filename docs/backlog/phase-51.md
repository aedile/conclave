# Phase 51 — Production Security Fixes
...
**Prerequisite**: Phase 48 merged (infrastructure fixes). Phase 50 preferred but
not blocking — these are production code fixes with their own test requirements.

**ADR**: T51.1 and T51.2 require new ADRs. T51.5 may require an ADR if
...
## T51.1 — DP Budget Deduction: Fail Closed
...
## T51.2 — Multi-Pod Audit Chain Integrity
...
## T51.3 — Default to Production Mode
...
## T51.4 — Pickle TOCTOU Mitigation
...
## T51.5 — Centralize Remaining Hardcodes
...
```
T51.1 (DP budget fail-closed) ─────────┐
T51.2 (Multi-pod audit chain) ─────────┼──> parallel (P0 security fixes)
T51.3 (Default to production mode) ────┘
                                          ↓ P0 fixes complete
T51.4 (Pickle TOCTOU mitigation) ──────┐
T51.5 (Centralize remaining hardcodes) ┼──> parallel (P1/P2 hardening)
                                        ┘
```

T51.1, T51.2, and T51.3 are independent P0 security fixes that can run in
parallel. T51.4 and T51.5 are lower-priority hardening tasks that can also run in
