...
| A-014 | Tenant isolation is application-level (`org_id` FK filtering in every query), not database-level (PostgreSQL Row-Level Security). A missed `WHERE org_id = :org_id` clause in any future query creates a cross-tenant data leak | Phase 79 spec (ADR-0065). Application-level isolation chosen for operational simplicity and verifiability at Tier 8 | High for current scale | Yes — integration tests in T79.3 validate isolation; architecture reviewer checks all new queries; red-team-reviewer runs on every phase | Verified by integration tests |
