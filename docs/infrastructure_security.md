...
The `docker-compose.mtls.yml` overlay mounts the `secrets/mtls/` directory into each
...
`shared/cert_metrics.py` (`conclave_cert_expiry_days` with a `service` label; TLS helpers in `shared/tls/config.py`).  Set
