...
def test_shared_models_subpackage_is_importable() -> None:
    ...
    """
    from synth_engine.shared import models  # noqa: F401
...
def test_get_current_user_with_valid_token(
    ...
    assert data["user_id"] == _USER_A_UUID
...
def test_get_current_user_passthrough_sentinel_values(
    ...
    assert data["org_id"] == _DEFAULT_ORG_UUID, (
        f"Pass-through org_id must be default sentinel {_DEFAULT_ORG_UUID}"
    )
    assert data["user_id"] == _DEFAULT_USER_UUID, (
        f"Pass-through user_id must be default sentinel {_DEFAULT_USER_UUID}"
    )
...
def test_get_current_user_rejects_missing_org_id_claim(
    ...
    """get_current_user rejects JWT without org_id claim (returns 401).

    T79.2 AC: verify_token() requires org_id claim in multi-tenant mode.
    A legacy JWT (no org_id) must not be accepted.
    """
    monkeypatch.setenv("JWT_SECRET_KEY", _TEST_SECRET)
    monkeypatch.setenv("JWT_ALGORITHM", "HS256")
    from synth_engine.shared.settings import get_settings
